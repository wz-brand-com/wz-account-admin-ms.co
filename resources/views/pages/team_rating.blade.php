                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <div class="form-group" id="name_form">
                                                        ...
                                                        <select name="rating_user_name" id="project_manager" class="w-100 p-2">
                                                ...
                                                <div class="col-md-6">
                                                    <div class="form-group" id="name_form">
                                                        ...
                                                        <select name="rating_manager_name" id="org_Based_project_manager" class="w-100 p-2">
                                                            <option value="" class="form-controll text-dark">Select Manager </option>
                                                            <option value="{{ Auth::user()->email }}"> {{ Auth::user()->email }}</option>
                                                        </select>
                                                    </div>
...
@push('js')
    ...
    <script>
        $(document).ready(function () {
            ...
            $.ajax({
                type: 'get',
                url: "{{ url('api/v1/j/invitedManagerUserGetting') }}/" +
                    userAuthId + '/' + organisation_id,
                dataType: "json",
                async: false,
                dataSrc: 'data',
                ...
                success: function (data) {
                    ...
                    $.each(data, function (i, link) {
                        $('#project_manager').append('<option name="' + link
                            .u_org_user_email + '" value="' + link.u_org_user_email +
                            '">' + link.u_org_user_email + '</option>');
                    });
            ...
            $.ajax({
                ...
                success: function (data) {
                    console.log('success main mamager id value a rha hai ki nahi' + data);
                    var p_name = ('#org_Based_project_manager');
                    // $(p_name).empty();

                    $.each(data, function (i, link) {
                        $('#org_Based_project_manager').append('<option name="' + link
                            .u_org_user_email + '" value="' + link.u_org_user_email +
                            '">' + link.u_org_user_email + '</option>');
                    });
