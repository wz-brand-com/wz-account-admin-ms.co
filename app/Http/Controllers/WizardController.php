...
class WizardController extends Controller
{
    ...
    public function createStepTwo(Request $request,$org_slug)
    {
        ...
        Log::info('org_id main  kiya aa rha hai');
        $org_user = Usersorganization::where('u_org_user_id',$user_id)->where('u_org_organization_id',$org_id)->first();
        $getting_slug_id = $org_user['u_org_organization_id'];
        $getting_slug_name = $org_user['u_org_slugname'];
        return view('user-wizard.create-step-two',compact('product','org_user','org_slug','getting_roll_id','getting_slug_id'));
    }
    ...
    public function postCreateStepTwo(Request $request,$org_slug)
    {
        ...
        $validatedData = $request->validate([
            ...
            'u_org_user_id' => 'required',
            'u_org_slugname' => 'required',
            'u_org_user_email' => 'required',
            'u_org_organization_id' => 'required',
        ]);
        // without validateData cannot data will be carry in next page
        $product = $request->session()->get('product');
        $product->fill($validatedData);
        $request->session()->put('product', $product);
    ...
    public function createStepThree(Request $request,$org_slug)
    {
        ...
        $org_user = Usersorganization::where('u_org_user_id',$user_id)->where('u_org_organization_id',$org_id)->first();
        $getting_slug_id = $org_user['u_org_organization_id'];
        return view('user-wizard.create-step-three',compact('product','org_user','org_slug','getting_roll_id','getting_slug_id'));
    }
    ...
    public function postCreateStepThree(Request $request,$org_slug)
    {
        ...
        Log::info('how many value is going to store in db' .print_r($product,true));
