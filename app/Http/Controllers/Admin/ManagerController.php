...
class ManagerController extends Controller
{
        ...
        if ($validator->fails()) {
            ...
            return response()->json($response,422);  
                ...
                if($check_email != NULL){
                    ...
                    // $verifycoursetoken = new Notexistmemberverifycoursetoken();
    ...
    public function member_active_deactive_user($id)
    {
        $user = Usersorganization::where('id',$id)->first();
        $user_status = $user->id;
        if($user->status == 0) {
        $user->status = '1';
    ...
    public function m_invited_removed_user($id)
    {
        Log::info(' m_invited_removed_user user removed ka id ka value aaa raha hai');
        $user = Usersorganization::where('id',$id)->first();
        $emailverify = $user['u_org_user_email'];
        log::info('verify ka email aaa gaya');
        if (is_null($user)) { 
        $response = [
        'success' => false,
        'data' => 'Empty',
        'message' => 'Oragination not found.'];
        return response()->json($response, 404);  
        } else {
        $user->delete();
        $varify_email =  DB::table("verifytokens")->where("email", $emailverify)->delete();
        Log::info("varify email deleted  ho gya"); 
        Log::info("user invited deleted  ho gya"); 
        $response = [
        'success' => true,
        'data' => $user,
    ...
    public function userMailIdValidation(){
