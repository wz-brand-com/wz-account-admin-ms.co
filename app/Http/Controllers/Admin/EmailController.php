...
//               return view('pages.email',compact('org_slug','getting_roll_id','a_user_api_bearer_token','slug_id')); 
//           } // first if close
//           if($getting_roll_id ==2){
//               return view('manager/email',compact('org_slug','getting_roll_id','a_user_api_bearer_token','slug_id'));
//           } // second if close
//           return view('user/email',compact('org_slug','getting_roll_id','a_user_api_bearer_token','slug_id'));
...
public function index($org_slug)
        ...
        // ========= project manager will be display who is assign for this slug name close
          if($getting_roll_id ==1){
            Log::info(' if ke ander role id le kr aaa rha hai tas page project controller');
              return view('pages.email',compact('org_slug','getting_roll_id','a_user_api_bearer_token','slug_id','project_managers','project_getting_user_manager')); 
          }
          if($getting_roll_id ==2){
              return view('manager/email',compact('org_slug','getting_roll_id','a_user_api_bearer_token','slug_id','project_managers'));
          } 
          return view('user/email',compact('org_slug','getting_roll_id','a_user_api_bearer_token','slug_id','project_managers'));
          }
    ...
    private function getEmailAccessToken(){
